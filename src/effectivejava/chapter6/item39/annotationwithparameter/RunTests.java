 ...
 *
 * 处理 @ExceptionTest 注解：对于每个带有 @ExceptionTest 注解的方法，尝试调用该方法。如果方法没有抛出异常或者抛出的异常类型不匹配 @ExceptionTest 注解指定的异常类型，则计算为失败，并打印失败信息。如果抛出的异常匹配 @ExceptionTest 注解指定的异常类型，则计算为通过。
 *
 * 这个测试运行器展示了如何使用反射来处理自定义注解，从而实现一个简单的测试框架。通过使用 @Test 和 @ExceptionTest 注解，可以轻松地标记要测试的方法以及期望的测试行为。
 */
// 程序处理标记注解和带参数的注解 (第 184 页)
